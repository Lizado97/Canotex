...
<body>
            ...
            <div class="grid grid-2">
                    ...
                    <div class="card">
                        ...
                        <div class="form-group">
                            ...
                            <div class="tipo-selector">
                                <div class="tipo-option active" id="opt-tradicional" onclick="cambiarTipoCortina('tradicional')">
                                    <div class="icon">🪟</div>
                                    <div class="title">Cortinas Tradicionales</div>
                                    <div class="desc">Estores, onda perfecta, fruncidas...</div>
                                </div>
                                <div class="tipo-option" id="opt-tecnica" onclick="cambiarTipoCortina('tecnica')">
                                    <div class="icon">⚙️</div>
                                    <div class="title">Cortinas Técnicas</div>
                                    <div class="desc">Enrollables, screen, blackout...</div>
                                </div>
                            </div>
                        </div>

                        <!-- Formulario Tradicional -->
                        <div id="form-tradicional">
                            <div class="form-group">
                            ...
                            <div class="checkbox-container">
                                ...
                                <label for="dos-piezas">✂️ Cortina en 2 piezas </label>
                            </div>
                        ...
                        <!-- Formulario Técnico -->
                        <div id="form-tecnico" class="hidden">
                            <div class="form-group">
                                <label>Tipo de Tejido Técnico *</label>
                                <select id="tejido-tecnico">
                                    <option value="">Selecciona el tejido técnico...</option>
                                </select>
                            </div>
                            <div class="grid grid-2">
                                <div class="form-group">
                                    <label>Ancho (cm) *</label>
                                    <input type="number" id="ancho-tecnico" placeholder="200">
                                </div>
                                <div class="form-group">
                                    <label>Alto (cm) *</label>
                                    <input type="number" id="alto-tecnico" placeholder="250">
                                </div>
                            </div>
                            <div class="form-group">
                                <label>Diámetro del Tubo *</label>
                                <select id="diametro-tubo">
                                    <option value="">Selecciona el diámetro...</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>Tipo de Accionamiento *</label>
                                <select id="accionamiento">
                                    <option value="">Selecciona el accionamiento...</option>
                                </select>
                            </div>
                            <button class="btn btn-primary" onclick="agregarItemTecnico()" style="width: 100%;">
                                ➕ Calcular y Agregar al Presupuesto
                            </button>
                        </div>
                    </div>
                ...
                <div>
                    <div class="resumen">
                        ...
                        <div class="checkbox-container" style="background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2);">
                            <input type="checkbox" id="incluir-instalacion" onchange="actualizarResumen()">
                            <label for="incluir-instalacion" style="color: white;">
                                Instalación <span id="texto-instalacion">()</span>
                            </label>
                        </div>
    ...
    <script>
       // TODOS LOS TEJIDOS ACTUALIZADOS DEL PDF 2025 (sin BAJA ni FIN EXIST.)
        const TEJIDOS = [
        ...
        const TEJIDOS_TECNICOS = [
            { nombre: "Screen 5%", tipo: "Screen", apertura: "5%", precio: 35.00 },
            { nombre: "Screen 3%", tipo: "Screen", apertura: "3%", precio: 38.00 },
            { nombre: "Screen 1%", tipo: "Screen", apertura: "1%", precio: 42.00 },
            { nombre: "Blackout Total", tipo: "Blackout", apertura: "0%", precio: 45.00 },
            { nombre: "Translúcido", tipo: "Translúcido", apertura: "50%", precio: 32.00 },
            { nombre: "Opaco", tipo: "Opaco", apertura: "0%", precio: 40.00 }
        ];

        const DIAMETROS_TUBO = [
            { diametro: "32mm", precioBase: 25.00, maxAncho: 150 },
            { diametro: "40mm", precioBase: 35.00, maxAncho: 200 },
            { diametro: "50mm", precioBase: 45.00, maxAncho: 300 },
            { diametro: "60mm", precioBase: 60.00, maxAncho: 400 }
        ];

        const TIPOS_ACCIONAMIENTO = [
            { nombre: "Cadena", precio: 15.00 },
            { nombre: "Motorizado con mando", precio: 120.00 },
            { nombre: "Motorizado con control remoto", precio: 150.00 },
            { nombre: "Motorizado wifi", precio: 180.00 }
        ];
       ...
       // TODOS LOS SISTEMAS
        const SISTEMAS = {
            ...
            "Guía Profesional Manual - Convencional": {100:40.99, 150:55.77, 200:69.05, 250:85.07, 300:99.30},
            "Guía Profesional Manual - Onda Perfecta": {100:43.15, 150:58.72, 200:72.68, 250:89.54, 300:104.54},
            "Guía Profesional a Cordón - Convencional": {100:52.53, 150:67.45, 200:81.01, 250:97.34, 300:111.90},
            "Guía Profesional a Cordón - Onda Perfecta": {100:55.30, 150:70.99, 200:85.27, 250:102.46, 300:117.80},
            "Barra Guía Cuadrada - Convencional": {100:65.77, 150:79.68, 200:93.59, 250:124.26, 300:138.18},
            "Barra Guía Cuadrada - Onda Perfecta": {100:67.78, 150:82.70, 200:97.61, 250:129.29, 300:144.21},
            "Barra Guía Redonda 20mm - Convencional": {100:64.51, 150:78.17, 200:91.81, 250:121.89, 300:135.54},
            "Barra Guía Redonda 20mm - Onda Perfecta": {100:66.49, 150:81.12, 200:95.75, 250:126.82, 300:141.46},
            "Barra Guía Redonda 28mm - Convencional": {100:74.18, 150:85.98, 200:100.99, 250:134.08, 300:149.09},
            "Barra Guía Redonda 28mm - Onda Perfecta": {100:76.46, 150:93.28, 200:105.32, 250:139.51, 300:155.60}
        };
        ...
        let itemsActuales = [];
        let tipoCortinaSistema = 'tradicional';

        document.addEventListener('DOMContentLoaded', function() {
            cargarSistemas();
            cargarTejidosTecnicos();
            cargarDiametrosTubo();
            cargarTiposAccionamiento();
            cargarDashboard();
        });

        function cambiarTipoCortina(tipo) {
            tipoCortinaSistema = tipo;
            document.getElementById('opt-tradicional').classList.remove('active');
            document.getElementById('opt-tecnica').classList.remove('active');
            
            if (tipo === 'tradicional') {
                document.getElementById('opt-tradicional').classList.add('active');
                document.getElementById('form-tradicional').classList.remove('hidden');
                document.getElementById('form-tecnico').classList.add('hidden');
            } else {
                document.getElementById('opt-tecnica').classList.add('active');
                document.getElementById('form-tradicional').classList.add('hidden');
                document.getElementById('form-tecnico').classList.remove('hidden');
            }
        }
        ...
        function cargarTejidosTecnicos() {
            const select = document.getElementById('tejido-tecnico');
            TEJIDOS_TECNICOS.forEach(t => {
                const option = document.createElement('option');
                option.value = t.nombre;
                option.textContent = `${t.nombre} - ${t.tipo} (Apertura ${t.apertura}) - ${t.precio}€/m²`;
                select.appendChild(option);
            });
        }

        function cargarDiametrosTubo() {
            const select = document.getElementById('diametro-tubo');
            DIAMETROS_TUBO.forEach(d => {
                const option = document.createElement('option');
                option.value = d.diametro;
                option.textContent = `${d.diametro} - Max. ancho: ${d.maxAncho}cm - Base: ${d.precioBase}€`;
                select.appendChild(option);
            });
        }

        function cargarTiposAccionamiento() {
            const select = document.getElementById('accionamiento');
            TIPOS_ACCIONAMIENTO.forEach(a => {
                const option = document.createElement('option');
        ...
        function buscarTejido() {
            ...
            searchResults.innerHTML = filtered.map(t => `
                <div class="search-result-item" onclick="seleccionarTejido('${t.nombre}')">
                    ...
                    <small style="color: #64748b;">${t.precio}€/ml - ${t.composicion}</small>
                </div>
            `).join('');
            searchResults.classList.remove('hidden');
        }
        ...
        function agregarItem() {
           ...
           if (tipo === 'estor_paquetto' || tipo === 'estor_plegable') {
                ...
                multiplicador = 2.4;
                tejidoMetros = (anchoParaTejido * multiplicador) / 100;
            ...
            const precioTotal = precioTejidoFinal + precioConfeccionFinal + precioSistema;
            ...
            const item = {
                ...
                precio_tejido: precioTejidoFinal,
                precio_confeccion: precioConfeccionFinal,
                precio_sistema: precioSistema
            };
        ...
        function agregarItemTecnico() {
            const ancho = parseFloat(document.getElementById('ancho-tecnico').value);
            const alto = parseFloat(document.getElementById('alto-tecnico').value);
            const tejidoNombre = document.getElementById('tejido-tecnico').value;
            const diametroNombre = document.getElementById('diametro-tubo').value;
            const accionamientoNombre = document.getElementById('accionamiento').value;

            if (!ancho || !alto || !tejidoNombre || !diametroNombre || !accionamientoNombre) {
                alert('Por favor completa todos los campos obligatorios');
                return;
            }

            const tejido = TEJIDOS_TECNICOS.find(t => t.nombre === tejidoNombre);
            const tubo = DIAMETROS_TUBO.find(d => d.diametro === diametroNombre);
            const accionamiento = TIPOS_ACCIONAMIENTO.find(a => a.nombre === accionamientoNombre);

            if (ancho > tubo.maxAncho) {
                alert(`El ancho máximo para tubo de ${diametroNombre} es ${tubo.maxAncho}cm`);
                return;
            }

            const metrosCuadrados = (ancho * alto) / 10000;
            const precioTejido = metrosCuadrados * tejido.precio;
            const precioTubo = tubo.precioBase * (ancho / 100);
            const precioAccionamiento = accionamiento.precio;
            const precioInstalacion = 50.00;
            const precioTotal = precioTejido + precioTubo + precioAccionamiento + precioInstalacion;

            const item = {
                id: Date.now(),
                tipo: 'cortina_tecnica',
                descripcion: `Cortina Técnica - ${tejido.nombre} - Tubo ${diametroNombre} - ${accionamiento.nombre}`,
                ancho, alto,
                tejido_nombre: tejido.nombre,
                precio_total: precioTotal,
                dos_piezas: false,
                precio_tejido: precioTejido,
                precio_confeccion: precioInstalacion,
                precio_sistema: precioTubo + precioAccionamiento
            };
            ...
            document.getElementById('ancho-tecnico').value = '';
            document.getElementById('alto-tecnico').value = '';
            document.getElementById('tejido-tecnico').value = '';
            document.getElementById('diametro-tubo').value = '';
            document.getElementById('accionamiento').value = '';
        }
            ...
            itemsActuales.forEach((item, index) => {
                ...
                div.innerHTML = `
                    <div class="item-info">
                        ...
                        <p>Medidas: ${item.ancho}cm x ${item.alto}cm ${item.dos_piezas ? '(2 piezas)' : ''}</p>
                    </div>
        ...
        function actualizarResumen() {
            ...
            const instalacion = incluirInstalacion ?(50 * totalItems): 0;
            const subtotalConInstalacion = subtotal + instalacion;
            ...
            if (totalItems > 0) {
                document.getElementById('texto-instalacion').textContent = ``;
            } else {
                document.getElementById('texto-instalacion').textContent = '';
            }
        }
        ...
        function guardarPresupuesto() {
            ...
            const subtotal = itemsActuales.reduce((sum, item) => sum + item.precio_total, 0);
            const incluirInstalacion = document.getElementById('incluir-instalacion').checked;
            const instalacion = incluirInstalacion ? (50 * itemsActuales.length) : 0;
            const total = (subtotal + instalacion) * 1.21;
            ...
            const presupuesto = {
                ...
                items: itemsActuales,
                total_sin_iva: subtotal + instalacion,
                total_con_iva: total,
                estado: 'borrador',
                notas: document.getElementById('notas').value,
                incluir_instalacion: incluirInstalacion,
                instalacion_total: instalacion
            };
        ...
        function verDetalle(id) {
            ...
            const instalacionTotal = p.instalacion_total || (p.incluir_instalacion ? (50 * p.items.length) : 0);
            document.getElementById('detalle-content').innerHTML = `
                <div class="aviso-orientativo">
                    <strong>⚠️ PRESUPUESTO ORIENTATIVO</strong>
                    <p>Precios sujetos a confirmación tras toma de medidas definitiva.</p>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 1rem; padding: 1rem; border-bottom: 2px solid #e2e8f0;">
                    ...
                    <div style="text-align: right;">
                        ...
                        <p style="font-size: 0.75rem;">Nº: ${p.numero}</p>
                    ...
                    <tbody>
                        ${p.items.map((item, i) => `
                            <tr>
                                ...
                                <td style="text-align: right; font-weight: 600;">${item.precio_total.toFixed(2)}€</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
                <div style="display: flex; justify-content: flex-end; margin-top: 1rem;">
                    <div style="width: 18rem;">
                        <div style="display: flex; justify-content: space-between; padding: 0.5rem 0; border-bottom: 1px solid #e2e8f0;">
                            <span>Subtotal:</span>
                            <strong>${(p.total_sin_iva - instalacionTotal).toFixed(2)}€</strong>
                        </div>
                        ${p.incluir_instalacion ? `
                        <div style="display: flex; justify-content: space-between; padding: 0.5rem 0; border-bottom: 1px solid #e2e8f0;">
                            <span>Instalación:</span>
                            <strong>${instalacionTotal.toFixed(2)}€</strong>
                        </div>
                        ` : ''}
                        <div style="display: flex; justify-content: space-between; padding: 0.5rem 0; border-bottom: 1px solid #e2e8f0;">
                            <span>IVA (21%):</span>
                            <strong>${(p.total_sin_iva * 0.21).toFixed(2)}€</strong>
                        </div>
                        <div style="display: flex; justify-content: space-between; padding: 1rem; background: #dbeafe; margin-top: 0.5rem; border-radius: 8px;">
                            ...
                            <span style="font-size: 1.5rem; font-weight: bold; color: #2563eb;">${p.total_con_iva.toFixed(2)}€</span>
                        </div>
                    </div>
                </div>
